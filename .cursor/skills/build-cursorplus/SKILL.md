...
```bash
xcodebuild -project "Cursor+.xcodeproj" -scheme "Cursor+" -configuration Debug -derivedDataPath build clean build 2>&1
```
...
```bash
xcodebuild -project "Cursor+.xcodeproj" -scheme "Cursor+" -configuration Release -derivedDataPath build clean build 2>&1
```
